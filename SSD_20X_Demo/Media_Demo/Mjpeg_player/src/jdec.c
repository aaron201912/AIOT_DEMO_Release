...
int jdec_decodeYUVFromBuf(char *jpgBuf, unsigned long jpgSize, jdecIMAGE *image, TANSFORM eTranOpt)
{
    //char *inFormat;
    ...
    if (NULL != jpegBuf)
    {
        ...
        if(inSubsamp == 0)
        {
            THROW_TJ("not support YUV type, YUV444");
        }
        //assert(image->phyAddr>0);
        //printf("width=%d height=%d yuvSize=%d inSubsamp=%d\n",width,height,yuvSize,inSubsamp);
...
int jdec_decodeYUV(char *fileName, jdecIMAGE *image, TANSFORM eTranOpt, SAMP esubsamp)
{
    char *inFormat;
    FILE *jpegFile = NULL;
    tjhandle tjInstance = NULL;
