...
int parse_marker(char *buf, long buf_size, int *image_width, int *image_height)
{
    ...
    long cnt = 0;
    int segment_len = 0;
    long tmp_buf_size = buf_size;
    char *tmp_buf = buf;
    for (;;)
    {
        if(tmp_buf_size > 0)
        {
            index = find_marker_index(tmp_buf, tmp_buf_size);
            //printf("find_marker ,tmp_buf_size=%ld buf_size=%ld\n",tmp_buf_size,buf_size-cnt);
        }
        else
        {
            printf("find_marker finish,tmp_buf_size=%ld  cnt=%ld\n",tmp_buf_size,cnt);
            return -1;
        }
        if (index < 0)
        {
            printf("parse_marker find no marker_index ,please check file index=%d tmp_buf_size=%ld\n",index,tmp_buf_size);
            return -1;
        }
        ...
        else if (buf_size < (cnt + segment_len + marker_len))
        {
            ...
            cnt += marker_len;
            tmp_buf += marker_len;
            tmp_buf_size -= marker_len;
        }
        else
        {
            cnt += (segment_len + marker_len);
            tmp_buf += (segment_len + marker_len);
            tmp_buf_size -= (segment_len + marker_len);
        }
    }
}
